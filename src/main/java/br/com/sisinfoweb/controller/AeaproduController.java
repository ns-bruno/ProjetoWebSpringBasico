...
import br.com.sisinfoweb.entity.AeaproduEntity;
import br.com.sisinfoweb.entity.SmadispoEntity;
import br.com.sisinfoweb.service.AeaproduService;
 ...
 */
@Controller
public class AeaproduController extends BaseMyController{
    
    @Autowired
    private AeaproduService aeaproduService;
    
    @RequestMapping(value = {"/Aeaprodu"}, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @Override
    public String initJson( Model model, 
        ...
        try{
            ...
            if ( ((sqlQuery != null) && (!sqlQuery.isEmpty())) || 
                ...
                listaPage = aeaproduService.findCustomNativeQueryClient(resume, sqlQuery, columnSelected, where, sort, pageable);
            
            } else {
                listaPage = aeaproduService.findAllClient(sort, pageable);
            }
            ...
            aeaproduService.closeEntityManager();
        }
    }
        
    @RequestMapping(value = {"/Aeaprrec"}, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    public String initAeaprrecJson( Model model, 
                            @RequestHeader() HttpHeaders httpHeaders, 
                            HttpServletResponse response, 
                            @RequestParam(name = PARAM_DISPOSITIVO, required = true) String dispositivo,
                            @RequestParam(name = PARAM_COLUMN_SELECTED, required = false) String columnSelected,
