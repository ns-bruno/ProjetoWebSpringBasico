...
import br.com.sisinfoweb.service.SmamenusService;
 ...
 */
@Controller
public class SmamenusController extends BaseMyController{

    @Autowired
    private SmamenusService smamenusService;
    
    @RequestMapping(value = {"/Smamenus"}, method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
    @ResponseBody
    @Override
    public String initJson( Model model, 
        ...
        try{
            ...
            smamenusService.setSmadispoEntity(smadispoEntity);
            
            PageBeans<SmamenusEntity> listaPage = null;
            ...
            if ( ((sqlQuery != null) && (!sqlQuery.isEmpty())) || 
                ...
                listaPage = smamenusService.findCustomNativeQueryClient(resume, sqlQuery, columnSelected, where, sort, pageable);
            
            } else {
                listaPage = smamenusService.findAllClient(sort, pageable);
            }
            ...
            smamenusService.closeDatabase();
        }
    }
    
    @RequestMapping(value = {"/Smpmenup"}, method = RequestMethod.GET)
    public ModelAndView init(Model model, @RequestHeader() HttpHeaders httpHeaders) {

        ModelAndView modelAndView = new ModelAndView("smpmenup/smpmenup");
